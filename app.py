import streamlit as st
import pandas as pd
import plotly.express as px
from vaderSentiment.vaderSentiment import SentimentIntensityAnalyzer
import praw # The Python Reddit API Wrapper

# --- Page Configuration ---
# This must be the first Streamlit command in your script.
st.set_page_config(
    page_title="Sentiment Analysis Dashboard",
    page_icon="📊",
    layout="wide",
    initial_sidebar_state="expanded"
)

# --- VADER Sentiment Analysis Setup ---
# Initialize the VADER sentiment intensity analyzer.
analyzer = SentimentIntensityAnalyzer()

# --- Core Functions ---

def get_vader_score(text):
    """Calculates the VADER compound score for a given text."""
    return analyzer.polarity_scores(text)['compound']

def classify_sentiment(score):
    """Classifies sentiment as Positive, Negative, or Neutral based on the VADER score."""
    if score >= 0.05:
        return 'Positive'
    elif score <= -0.05:
        return 'Negative'
    else:
        return 'Neutral'

# --- Data Loading and Processing Functions ---

@st.cache_data
def load_and_process_airline_data(file_path):
    """
    Loads the static airline tweet dataset and performs sentiment analysis.
    This function is cached to improve performance.
    """
    df = pd.read_csv(file_path)
    df.dropna(subset=['text'], inplace=True)
    df['vader_score'] = df['text'].apply(get_vader_score)
    df['vader_sentiment'] = df['vader_score'].apply(classify_sentiment)
    return df

@st.cache_data(ttl=600) # Cache the results for 10 minutes
def fetch_and_analyze_reddit_comments(subreddit_name, num_posts=25):
    """
    Fetches and analyzes recent comments from a specified subreddit using PRAW.
    Requires Reddit API credentials to be set in Streamlit's secrets.
    """
    try:
        # Initialize PRAW with credentials stored securely in Streamlit's secrets manager.
        reddit = praw.Reddit(
            client_id=st.secrets["REDDIT_CLIENT_ID"],
            client_secret=st.secrets["REDDIT_CLIENT_SECRET"],
            user_agent=f"SentimentAnalysisApp by u/{st.secrets.get('REDDIT_USERNAME', 'default_user')}"
        )
        
        subreddit = reddit.subreddit(subreddit_name)
        comments_list = []
        
        # Scrape a limited number of comments from the 'hot' section for analysis.
        for submission in subreddit.hot(limit=num_posts):
            # `replace_more(limit=0)` prevents fetching deeper comment threads.
            submission.comments.replace_more(limit=0)
            for comment in submission.comments.list()[:5]: # Get top 5 comments per post
                if comment.body and comment.body not in ['[deleted]', '[removed]']:
                    comments_list.append(comment.body)

        if not comments_list:
            st.warning("No comments found. The subreddit may be new, restricted, or have low activity.")
            return pd.DataFrame()
            
        df_live = pd.DataFrame(comments_list, columns=['text'])
        
        # Perform sentiment analysis on the fetched comments.
        df_live['vader_score'] = df_live['text'].apply(get_vader_score)
        df_live['vader_sentiment'] = df_live['vader_score'].apply(classify_sentiment)
        
        return df_live
    except Exception as e:
        st.error(f"An error occurred: {e}. Please check the subreddit name and your credentials.")
        return pd.DataFrame()

# --- Main Application UI ---

# Load the static dataset for the demo mode.
df_airline = load_and_process_airline_data('Tweets.csv')

# --- Sidebar ---
with st.sidebar:
    st.title("⚙️ Control Panel")
    app_mode = st.radio(
        "Choose Analysis Mode",
        ("Airline Tweet Demo", "Live Reddit Analysis")
    )
    st.divider()
    
    st.header("About This Project")
    st.info(
        "This dashboard uses NLP to analyze sentiment from social media. "
        "It features a demo with a static dataset and a live analysis mode using the Reddit API."
    )
    st.write("Created by: **Syed Moin Uddin**")
    st.write("[View on GitHub](https://github.com/imSyedMoinUddin/Sentiment-Analysis-Dashboard)")

# --- Header Section ---
st.title("📊 Social Media Sentiment Analysis")
st.markdown(f"Currently in **{app_mode}** mode.")
st.divider()

# --- Mode 1: Airline Tweet Demo ---
if app_mode == "Airline Tweet Demo":
    st.header("US Airline Tweet Sentiment")
    airline_choice = st.selectbox(
        "Choose an Airline:",
        options=['All'] + sorted(df_airline['airline'].unique().tolist())
    )
    if airline_choice != 'All': 
        filtered_df = df_airline[df_airline['airline'] == airline_choice]
    else: 
        filtered_df = df_airline
    st.write(f"Displaying results for: **{airline_choice}**")

# --- Mode 2: Live Reddit Analysis ---
elif app_mode == "Live Reddit Analysis":
    st.header("Live Subreddit Sentiment Analysis")
    subreddit_name = st.text_input("Enter a subreddit name (e.g., technology, gaming, python):", "Python")
    
    if st.button("Analyze Subreddit"):
        with st.spinner(f"Fetching and analyzing recent comments from r/{subreddit_name}..."):
            filtered_df = fetch_and_analyze_reddit_comments(subreddit_name)
    else:
        # Create an empty DataFrame to prevent errors before the button is clicked.
        filtered_df = pd.DataFrame()

# --- Universal Display Section for Analysis Results ---
# This section runs if 'filtered_df' exists and is not empty.
if 'filtered_df' in locals() and not filtered_df.empty:
    st.subheader("Results")
    
    sentiment_counts = filtered_df['vader_sentiment'].value_counts().reset_index()
    sentiment_counts.columns = ['sentiment', 'count']
    color_map = {'Positive': '#2ca02c', 'Negative': '#d62728', 'Neutral': '#7f7f7f'}

    # Layout for metrics and chart
    col1, col2 = st.columns([1, 2])
    with col1:
        st.subheader("Overall Sentiment")
        total_posts = len(filtered_df)
        positive_pct = (sentiment_counts.get('count', 0)[sentiment_counts['sentiment'] == 'Positive'].sum() / total_posts) * 100
        negative_pct = (sentiment_counts.get('count', 0)[sentiment_counts['sentiment'] == 'Negative'].sum() / total_posts) * 100
        neutral_pct = (sentiment_counts.get('count', 0)[sentiment_counts['sentiment'] == 'Neutral'].sum() / total_posts) * 100
        
        st.metric(label="Positive", value=f"{positive_pct:.2f}%")
        st.metric(label="Negative", value=f"{negative_pct:.2f}%")
        st.metric(label="Neutral", value=f"{neutral_pct:.2f}%")
    with col2:
        st.subheader("Sentiment Distribution")
        fig = px.pie(sentiment_counts, names='sentiment', values='count', hole=0.4, color='sentiment', color_discrete_map=color_map)
        st.plotly_chart(fig, use_container_width=True)
    
    st.divider()

    st.subheader("Explore the Raw Data")
    with st.expander("Click to view individual posts and their scores"):
        def color_sentiment(sentiment):
            """Applies background color to the sentiment column for better readability."""
            if sentiment == 'Positive': return 'background-color: #e6fffa'
            elif sentiment == 'Negative': return 'background-color: #ffe6e6'
            else: return 'background-color: #f0f2f6'
        
        styled_df = filtered_df[['text', 'vader_score', 'vader_sentiment']].style.map(color_sentiment, subset=['vader_sentiment'])
        st.dataframe(styled_df, use_container_width=True, height=400)
